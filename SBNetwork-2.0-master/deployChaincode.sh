...
setGlobalsForPeer0Org1() {
    ...
    # export CORE_PEER_MSPCONFIGPATH=${PWD}/artifacts/channel/crypto-config/peerOrganizations/org1.smartbagng.com/peers/peer0.org1.smartbagng.com/msp
...
approveForMyOrg1() {
    ...
    peer lifecycle chaincode approveformyorg -o localhost:7050 \
        --ordererTLSHostnameOverride orderer.smartbagng.com --tls \
        --collections-config $PRIVATE_DATA_CONFIG \
...
# --peerAddresses peer0.org1.smartbagng.com:7051 --tlsRootCertFiles $PEER0_ORG1_CA --peerAddresses peer0.org2.smartbagng.com:9051 --tlsRootCertFiles $PEER0_ORG2_CA
...
approveForMyOrg2() {
    ...
    peer lifecycle chaincode approveformyorg -o localhost:7050 \
        --ordererTLSHostnameOverride orderer.smartbagng.com --tls $CORE_PEER_TLS_ENABLED \
...
commitChaincodeDefination() {
    setGlobalsForPeer0Org1
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.smartbagng.com \
...
chaincodeInvoke() {
    # setGlobalsForPeer0Org1
    # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.smartbagng.com \
...
chaincodeQuery() {
    ...
    # Query all cars
    # peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["queryAllBag"]}'

    # Query Car by Id
     peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryBag","Args":["CAR0"]}'
