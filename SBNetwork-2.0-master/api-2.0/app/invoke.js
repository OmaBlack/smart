...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name) => {
    try {
        ...
        if (fcn === "createBag") {
            ...
            message = `Successfully added the bag asset with key ${args[0]}`

        } else if (fcn === "changeBagOwner") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully changed Bag owner with key ${args[0]}`
        }
		else if (fcn === "createScreening") { 
            ...
            return `Invocation require either createBag or changeBagOwner as function but got ${fcn}`
        }
